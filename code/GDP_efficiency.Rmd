...
data_GDP_physicians <- 
  data_all %>% 
    cross_section(start_average = 1960, end_average = 1980, year_outcome = 1980, outcome_variable = "physicians") %>%
  former_states() %>% 
  update_regime_types(granularity = 4, reference_regime = "capitalist_periphery", regime_vietnam = "socialist") %>% 
  drop_na()
write_csv(data_GDP_physicians, "../data/aggregated_datasets/data_GDP_physicians.csv")

writeLines("\nSocialist countries included:")
data_GDP_physicians %>% filter(regime_type == "socialist") %>% pull(country_name)
```

### logarithmic curves

```{r}
model_GDP_physicians <- lm(physicians ~ regime_type*log(GDP_PPP), 
                        data = data_GDP_physicians)
vcov_GDP_physicians <- vcovHC(model_GDP_physicians, type = "HC3")

parameters(model_GDP_physicians, vcov = vcov_GDP_physicians)
```
```{r}
summary(model_GDP_physicians)
```


```{r}
plot(model_GDP_physicians)
```

```{r}
ggintplot_categorical_moderator(obj = model_GDP_physicians, 
                                vcov = vcov_GDP_physicians,
                                pred = "log(GDP_PPP)", 
                                categorical_moderator = "regime_type", 
                                levels = list("capitalist_periphery", "capitalist_core", "revolutionary", "socialist"),
                                title = TRUE)
...
```{r}
ggintplot(obj = model_GDP_physicians, 
          vcov = vcov_GDP_physicians,
          varnames = c("regime_typesocialist", "log(GDP_PPP)"), varlabs=c("socialist regime type", "GDP ($ PPP)"),
          title = TRUE, 
          rug = TRUE, 
...
```{r}
interact_plot_custom(model_GDP_physicians, 
              vcov = vcov_GDP_physicians,
              pred = "GDP_PPP",
              
              data = data_GDP_physicians, insignificant_lines = "dashed", x.label = "GDP (PPP)", y.label = "Physicians per 1000 people", 
              ) 
```
...
### saturation curves
```{r}
sat_GDP_physicians <- data_GDP_physicians$physicians %>% max() * 1.1
model_GDP_physicians_sat <- lm(log(sat_GDP_physicians - physicians) ~ regime_type*log(GDP_PPP),
                        data = data_GDP_physicians)
vcov_GDP_physicians_sat <- vcovHC(model_GDP_physicians_sat, type = "HC3")

summary(model_GDP_physicians_sat)
```
...
interact_plot_custom(model_GDP_female_labour, 
                                vcov = vcov_GDP_female_labour, pred = "GDP_PPP",data = data_GDP_female_labour, insignificant_lines = "dashed", x.label = "GDP (PPP)", y.label = "Female labour force share", ) 
```
...
```{r}
data_GDP_adolescent_fertility <- 
  data_all %>% 
  cross_section(start_average = 1960, end_average = 1980, year_outcome = 1980, outcome_variable = "adolescent_fertility") %>% 
  former_states() %>% 
  update_regime_types(granularity = 4, reference_regime = "capitalist_periphery", regime_vietnam = "socialist") %>% 
  drop_na()
write_csv(data_GDP_adolescent_fertility, "../data/aggregated_datasets/data_GDP_adolescent_fertility.csv")


writeLines("\nSocialist countries included:")
data_GDP_adolescent_fertility %>% filter(regime_type == "socialist") %>% pull(country_name)
```

### logarithmic curves

```{r}
model_GDP_adolescent_fertility <- lm(adolescent_fertility ~ regime_type*log(GDP_PPP), 
                        data = data_GDP_adolescent_fertility)

vcov_GDP_adolescent_fertility <- vcovHC(model_GDP_adolescent_fertility, type = "HC3")

parameters(model_GDP_adolescent_fertility, vcov = vcov_GDP_adolescent_fertility)
```

```{r}
ggintplot_categorical_moderator(obj = model_GDP_adolescent_fertility, 
                                vcov = vcov_GDP_adolescent_fertility,
                                pred = "log(GDP_PPP)", 
                                categorical_moderator = "regime_type", 
                                levels = list("capitalist_periphery", "capitalist_core", "revolutionary", "socialist"),
                                title = TRUE)
...
```{r}
ggintplot(obj = model_GDP_adolescent_fertility, 
                                vcov = vcov_GDP_adolescent_fertility, varnames = c("regime_typesocialist", "log(GDP_PPP)"), varlabs=c("socialist regime type", "GDP ($ PPP)"),
          title = TRUE, 
          rug = TRUE, 
          twoways = FALSE, 
          rugwidth = 0.5,
...
```{r}
ggintplot(obj = model_GDP_adolescent_fertility, 
                                vcov = vcov_GDP_adolescent_fertility, varnames = c("regime_typecapitalist_core", "log(GDP_PPP)"), varlabs=c("capitalist core regime type", "GDP (PPP, log-transformed)"),
          title = TRUE, 
          rug = TRUE,
          rugwidth = 0.5,
          unlog_x = TRUE,
...
```{r}
interact_plot_custom(model_GDP_adolescent_fertility, 
                                vcov = vcov_GDP_adolescent_fertility, pred = "GDP_PPP",data = data_GDP_adolescent_fertility, insignificant_lines = "dashed", x.label = "GDP (PPP)", y.label = "Adolescent fertility rate", ) 
```


### only regime type
```{r}
model_regime_adolescent_fertility <- lm(adolescent_fertility ~ regime_type, 
                        data = data_GDP_adolescent_fertility)

vcov_regime_adolescent_fertility <- vcovHC(model_regime_adolescent_fertility, type = "HC3")

parameters(model_regime_adolescent_fertility, vcov = vcov_regime_adolescent_fertility)
```
...
interact_plot_physicians <- do.call(interact_plot_custom, c(list(model_GDP_physicians, vcov_GDP_physicians, data_GDP_physicians, y.label = "Physicians per 1000 people"), argument_list_GDP))
...
interact_plot_female_labour <- do.call(interact_plot_custom, c(list(model_GDP_female_labour, vcov_GDP_female_labour, data_GDP_female_labour, y.label = "Female labour force share", percentage = TRUE), argument_list_GDP))
...
interact_plot_adolescent_fertility <- do.call(interact_plot_custom, c(list(model_GDP_adolescent_fertility, vcov_GDP_adolescent_fertility, data_GDP_adolescent_fertility, y.label = "Adolescent fertility rate", percentage = TRUE, reverse = TRUE), argument_list_GDP))
```
...
generate_supplementary_information(model = model_GDP_physicians, vcov = vcov_GDP_physicians, model_sat = model_GDP_physicians_sat, vcov_sat = vcov_GDP_physicians_sat, data = data_GDP_physicians, pred = "GDP_PPP", resp = "physicians", y.label = "Physicians per 1000 people", percentage = FALSE, reverse = FALSE, sat = sat_GDP_physicians, y_max = NA)
...
generate_supplementary_information(model = model_GDP_female_labour, vcov = vcov_GDP_female_labour, data = data_GDP_female_labour, pred = "GDP_PPP", resp = "female_labour", y.label = "Female labour force share", percentage = TRUE, reverse = FALSE, y_max = NA)
...
generate_supplementary_information(model = model_GDP_adolescent_fertility, vcov = vcov_GDP_adolescent_fertility, data = data_GDP_adolescent_fertility, pred = "GDP_PPP", resp = "adolescent_fertility", y.label = "Adolescent fertility rate", percentage = TRUE, reverse = TRUE, y_max = NA)
